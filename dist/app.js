// 后端地址来自 config.js（window.API_BASE）。留空 = 同源相对路径。
// 静态站放在子目录时也安全：'/' 开头的路径不写死域名，而是拼在 API_BASE 后面。
const API_BASE=(typeof window!=='undefined' && window.API_BASE || '').replace(/\/+$/,'');
const FALLBACK_TITLE=(typeof window!=='undefined' && window.FALLBACK_TITLE)||'宿舍电费看板';
...
async function api(path,opt={}){
  let r=null;
  try{
    r=await fetch(API_BASE+path,{cache:'no-store',...opt});
  }catch(e){
    // 网络层就失败了。跨域被拦也是这条：浏览器不会让 JS 看到详情。
    return {ok:false,error:'连不上后端（'+API_BASE+'）。检查 config.js 里的地址，以及后端 cors_origins 有没有放行本站。'};
  }
...
async function boot(){
  ...
  CFG=d.config||{}; LAST=d;
  const bound=!!d.bound;
  const title=CFG.site_title||FALLBACK_TITLE;
  document.title=title; q('#h1').textContent=title;
...
async function forgetMine(){
  if(!window.confirm('确定删除你的全部数据吗？包括服务端保存的会话凭据。删除后需要重新扫码。')) return;
